...
app.get("jokes/:id", (req, res) => {
  ...
  const foundJoke = jokes.find((joke) => joke.id === paramId);
  res.json(foundJoke);
});
...
app.put("jokes/:id", (req, res) => {
  ...
  const searchIndex = jokes.find((joke) => joke.id === paramId);
...
app.patch("jokes/:id", (req, res) => {
  const paramId = parseInt(req.params.id);
  const existingJoke = jokes.find((joke) => joke.id === paramId);
...
app.delete("jokes/:id", (req, res) => {
  ...
  if(searchIndex > -1) {
    ...
    res 
     .status(404)
     .json({error: "Joke with the id " + paramId + "not found."});
  }
});
//8. DELETE All jokes
app.delete("/all", (req, res) => {
  const useaKey = req.query.key;
  if(useaKey === masterKey){
    jokes = [];
    res.status(200);
  }
  else {
    res
    .status(404)
    .json({error: "Error you are not authorized to perform this action."});
  }
})
